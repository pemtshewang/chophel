...
export default async function BrowsePage() {
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex flex-col relative overflow-hidden max-w-md mx-auto shadow-2xl">
      <header className="sticky top-0 z-30 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-b border-stone-200 dark:border-stone-800">
        <div className="px-4 pt-12 pb-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-primary-crimson text-3xl">local_library</span>
              <h1 className="text-2xl font-bold tracking-tight text-wood dark:text-stone-200">Chöpel Library</h1>
            </div>
          </div>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 material-symbols-outlined">search</span>
            <input
              className="w-full pl-10 pr-4 py-3 rounded-xl border border-stone-200 bg-white dark:bg-stone-900 dark:border-stone-700 outline-none text-sm"
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-24 pt-4 space-y-4">
        {prayers.map((prayer) => (
          <Link href={`/reader/${prayer.id}`} key={prayer.id}>
            <article className="relative bg-parchment dark:bg-stone-800 rounded-xl border border-gold/40 dark:border-gold/20 shadow-sm p-4 flex gap-4 transition-transform active:scale-[0.99] mb-4">
              ...
              <div className="flex-1 flex flex-col gap-1">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-lg text-wood dark:text-stone-100 leading-tight">{prayer.title}</h3>
                    <p className="text-tibetan text-base text-wood/80 dark:text-stone-300 mt-1">{prayer.tibetan}</p>
                  </div>
                </div>

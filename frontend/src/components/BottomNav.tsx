...
const BottomNav = ({ activeTab }: { activeTab: string }) => {
  return (
    <>
      ...
      <nav className="fixed z-50 left-0 right-0 bottom-0 bg-white/95 dark:bg-[#f3ece2]/95 backdrop-blur-md border-t border-stone-200 dark:border-[#dfd1bf] px-2 pt-2 pb-safe md:hidden">
      ...
      <nav className="hidden md:block fixed top-20 left-1/2 -translate-x-1/2 z-40">
        <div className="flex items-center gap-1 rounded-2xl border border-stone-200/80 dark:border-[#d9cab8] bg-white/90 dark:bg-[#f3ece2]/90 backdrop-blur-md shadow-lg p-1.5">
          {navItems.map((item) => {
            const isActive = activeTab === item.key;
